#include "displayapp/screens/WatchFaceGirliePopCasio.h"
...
WatchFaceGirliePopCasio::WatchFaceGirliePopCasio(Controllers::DateTime& dateTimeController,
...
WatchFaceGirliePopCasio::~WatchFaceGirliePopCasio() {
...
void WatchFaceGirliePopCasio::Refresh() {
  ...
  if (heartbeat.IsUpdated() || heartbeatRunning.IsUpdated()) {
    if (heartbeatRunning.Get()) {
      ...
      lv_obj_set_style_local_text_color(heartbeatIcon, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0xFFBDFF));	//dim color when hearbeat is off
      lv_label_set_text_static(heartbeatValue, "");
    }
...
bool WatchFaceGirliePopCasio::IsAvailable(Pinetime::Controllers::FS& filesystem) {
  lfs_file file = {};
